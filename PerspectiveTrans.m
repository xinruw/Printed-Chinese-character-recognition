function [imgn]=PerspectiveTrans(img)
%返回透视变换的结果，img是灰度图
%用户取四个点，依次是左上、右上、左下、右下，按回车结束选取
...
%新的顶点，这里取矩形
Y=round([dot(1,1) dot(1,1) dot(1,1)+h dot(1,1)+h]);     
X=round([dot(1,2) dot(1,2)+w dot(1,2) dot(1,2)+w]);
...
for i = Y(1):Y(3)                       %从变换图像中反向寻找原图像的点，以免出现空洞，和旋转放大原理一样
    for j = X(1):X(2)
...
imgn=imgn(Y(1)+delta_y:Y(3)+delta_y,X(1)+delta_x:X(2)+delta_x);
imgn=uint8(imgn);
